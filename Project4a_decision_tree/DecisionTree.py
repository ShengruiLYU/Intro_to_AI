...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    while currnode.isleaf == False:
        ...
        testingAttr= currnode.attr

        testingvalue = classificationData[testingAttr]

        currnode = currnode.children[testingvalue]
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for i in range(len(attrValues)):
        subsetexamples = getPertinentExamples(examples,attrName,attrValues[i])
        subsetclasscount  = getClassCounts(subsetexamples,className)
        attributeCounts[attrValues[i]] = subsetclasscount
...
def setEntropy(classCounts):
    ...
    for i in range(len(classCounts)):
        probability = classCounts[i]/(totalcount*1.0)
        
        if (probability == 0):
            log2 = 0
        else:
            log2 = log(probability)/log(2)
...
def remainder(examples,attrName,attrValues,className):
    ...
    for attributevalue in attributecounts :
        
        classcountslist = []
        attributetotal = 0

        for labels in attributecounts[attributevalue]:
            classcountslist.append(attributecounts[attributevalue][labels])
            attributetotal += attributecounts[attributevalue][labels]
        entropy = setEntropy(classcountslist)

        remainder += (attributetotal/(totalexamples*1.0))*entropy
...
def infoGain(examples,attrName,attrValues,className):
    ...
    for label in classcount:
        classcountslist.append(classcount[label])

    currentEntropy = setEntropy(classcountslist)
    re = remainder(examples,attrName,attrValues,className)

    infogain = currentEntropy - re
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for attributevalue in attributecounts :
        ...
        for labels in attributecounts[attributevalue]:
            classcountslist.append(attributecounts[attributevalue][labels])
            attributetotal += attributecounts[attributevalue][labels]
        gini = giniIndex(classcountslist)

        gain += (attributetotal/(totalexamples*1.0))*gini
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    if len(examples)== 0 :
        ...
        classcount= getClassCounts(examples,className)
        mostclass = getMostCommonClass(examples,className)
        defaultLabel = mostclass
       
        
        if len(examples) == classcount[mostclass] :
            return LeafNode(mostclass)
    ...
    for attribute in remainingAttributes :
        infogain = gainFunc(examples,attribute,attributeValues[attribute],className)
    ...
    newtree = Node(bestattribute)
    bestAttrvalues = attributeValues[bestattribute]

    newAttrValues = {i:attributeValues[i] for i in attributeValues if i!= bestattribute}

    newremaining = [x for x in remainingAttributes if x != bestattribute]

    totalValue = len(examples)
    ...
    for value in bestAttrvalues :
        ...
        childnode = makeSubtrees(newremaining,subset,newAttrValues,className,defaultLabel,setScoreFunc,gainFunc)

        newtree.children[value] = childnode
