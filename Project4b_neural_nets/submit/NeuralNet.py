...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        sigact = self.sigmoidActivation(inActs)
        return(sigact*(1.0 - sigact))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(self.inSize):
            modification = alpha*delta*temp[i]
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layercount in range(self.numLayers):
            #size = self.layerSize[layercount+1]
            size = len(self.layers[layercount])
            

            layerResult = []

            for percepcount in range(size):
                
                percepResult = self.layers[layercount][percepcount].sigmoidActivation(resultlist[layercount])
                
                layerResult.append(percepResult)

            
            resultlist.append(layerResult)
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    tempsum = 0
                    for node in range(len(nextLayer)):
                        w = nextLayer[node].weights[neuronNum+1]
                        tempsum += w*deltas[0][node]
                    
                    delta = gPrime*tempsum
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        ...
        lastLayerOutput = [round(y) for y in allLayerOutput[-1]]
        isCorrect = True

        for i in range(len(example[1])):            
            if (lastLayerOutput[i] != example[1][i]):
                isCorrect = False
                break
        if isCorrect :
            testCorrect += 1
